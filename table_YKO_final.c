...
// Define variables
  ...
  unsigned int *R1=calloc(N1,sizeof(unsigned int));
  unsigned int *R2=calloc(N2,sizeof(unsigned int));
  unsigned int *R3=calloc(N3,sizeof(unsigned int));
  unsigned int *R4=calloc(N4,sizeof(unsigned int));
  unsigned int *R5=calloc(N5,sizeof(unsigned int));
  unsigned int *R6=calloc(N6,sizeof(unsigned int));
  unsigned int *R7=calloc(N7,sizeof(unsigned int));
  unsigned int *R8=calloc(N8,sizeof(unsigned int));
  unsigned int *R9=calloc(N9,sizeof(unsigned int));
  unsigned int *R10=calloc(N10,sizeof(unsigned int));
  unsigned int *R11=calloc(N11,sizeof(unsigned int));
  unsigned int *R12=calloc(N12,sizeof(unsigned int));
  unsigned int *R13=calloc(N13,sizeof(unsigned int));
  unsigned int *R14=calloc(N14,sizeof(unsigned int));
  unsigned int *R15=calloc(N15,sizeof(unsigned int));
  unsigned int *R16=calloc(N16,sizeof(unsigned int));
  unsigned int *R17=calloc(N17,sizeof(unsigned int));
  unsigned int *R18=calloc(N18,sizeof(unsigned int));
  unsigned int *R19=calloc(N19,sizeof(unsigned int));
  unsigned int *R20=calloc(N20,sizeof(unsigned int));
  unsigned int *R21=calloc(N21,sizeof(unsigned int));
  unsigned int *R22=calloc(N22,sizeof(unsigned int));
  unsigned int *R23=calloc(N23,sizeof(unsigned int));
  unsigned int *R24=calloc(N24,sizeof(unsigned int));

// Save all inputs to memory
  fprintf(stderr,"Starting to read input files into memory...\n");
  for (i=0;i<N1;i++) {
    fgets(line,10000,S1);
    sscanf(line,"%u %s",&R1[i],G1+20*i);
  }
  for (i=0;i<N2;i++) {
    fgets(line,20000,S2);
    sscanf(line,"%u %s",&R2[i],G2+20*i);
  }
  for (i=0;i<N3;i++) {
    fgets(line,30000,S3);
    sscanf(line,"%u %s",&R3[i],G3+20*i);
  }
  for (i=0;i<N4;i++) {
    fgets(line,40000,S4);
    sscanf(line,"%u %s",&R4[i],G4+20*i);
  }
  for (i=0;i<N5;i++) {
    fgets(line,50000,S5);
    sscanf(line,"%u %s",&R5[i],G5+20*i);
  }
  for (i=0;i<N6;i++) {
    fgets(line,60000,S6);
    sscanf(line,"%u %s",&R6[i],G6+20*i);
  }
  for (i=0;i<N7;i++) {
    fgets(line,70000,S7);
    sscanf(line,"%u %s",&R7[i],G7+20*i);
  }
  for (i=0;i<N8;i++) {
    fgets(line,80000,S8);
    sscanf(line,"%u %s",&R8[i],G8+20*i);
  }
  for (i=0;i<N9;i++) {
    fgets(line,90000,S9);
    sscanf(line,"%u %s",&R9[i],G9+20*i);
  }
  for (i=0;i<N10;i++) {
    fgets(line,100000,S10);
    sscanf(line,"%u %s",&R10[i],G10+20*i);
  }
  for (i=0;i<N11;i++) {
    fgets(line,110000,S11);
    sscanf(line,"%u %s",&R11[i],G11+20*i);
  }
  for (i=0;i<N12;i++) {
    fgets(line,120000,S12);
    sscanf(line,"%u %s",&R12[i],G12+20*i);
  }
  for (i=0;i<N13;i++) {
    fgets(line,130000,S13);
    sscanf(line,"%u %s",&R13[i],G13+20*i);
  }
  for (i=0;i<N14;i++) {
    fgets(line,140000,S14);
    sscanf(line,"%u %s",&R14[i],G14+20*i);
  }
  for (i=0;i<N15;i++) {
    fgets(line,150000,S15);
    sscanf(line,"%u %s",&R15[i],G15+20*i);
  }
  for (i=0;i<N16;i++) {
    fgets(line,160000,S16);
    sscanf(line,"%u %s",&R16[i],G16+20*i);
  }
  for (i=0;i<N17;i++) {
    fgets(line,170000,S17);
    sscanf(line,"%u %s",&R17[i],G17+20*i);
  }
  for (i=0;i<N18;i++) {
    fgets(line,180000,S18);
    sscanf(line,"%u %s",&R18[i],G18+20*i);
  }
  for (i=0;i<N19;i++) {
    fgets(line,190000,S19);
    sscanf(line,"%u %s",&R19[i],G19+20*i);
  }
  for (i=0;i<N20;i++) {
    fgets(line,200000,S20);
    sscanf(line,"%u %s",&R20[i],G20+20*i);
  }
  for (i=0;i<N21;i++) {
    fgets(line,210000,S21);
    sscanf(line,"%u %s",&R21[i],G21+20*i);
  }
  for (i=0;i<N22;i++) {
    fgets(line,220000,S22);
    sscanf(line,"%u %s",&R22[i],G22+20*i);
  }
  for (i=0;i<N23;i++) {
    fgets(line,230000,S23);
    sscanf(line,"%u %s",&R23[i],G23+20*i);
  }
  for (i=0;i<N24;i++) {
    fgets(line,240000,S24);
    sscanf(line,"%u %s",&R24[i],G24+20*i);
  }
